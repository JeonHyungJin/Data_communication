...
#socket모듈의 함수를 이용해서 Bytes를 IP주소 형식으로 출력한다.
		...
		final_ipv4 = 14 + IP_header_length
		...
		if(Protocol == 6):
			...
			TCP_header = struct.unpack('!2s2s4s4s1s1s2s2s2s',packet[0][final_ipv4:final_ipv4+20])
			TCP_source_port = int(TCP_header[0].hex(),16)
			TCP_destination_port = int(TCP_header[1].hex(),16)
			TCP_sequence_num = int(TCP_header[2].hex(), 16)
			TCP_ack_num = int(TCP_header[3].hex(), 16)
			TCP_data_offset = (TCP_header[4][0] & 11110000) >> 4
			TCP_reserved = (TCP_header[4][0] & 14) >> 1
			TCP_ns = TCP_header[4][0] & 1
			TCP_cwr = (TCP_header[5][0] & 10000000) >> 7
			TCP_ece = (TCP_header[5][0] & 64) >> 6
			TCP_urg = (TCP_header[5][0] & 32) >> 5
			TCP_ack = (TCP_header[5][0] & 16) >> 4
			TCP_psh = (TCP_header[5][0] & 8) >> 3
			TCP_pst = (TCP_header[5][0] & 4) >> 2
			TCP_syn = (TCP_header[5][0] & 2) >> 1
			TCP_fin = TCP_header[5][0] & 1
			TCP_win_size = int(TCP_header[6].hex(), 16)
			TCP_checksum = int(TCP_header[7].hex(), 16)
			TCP_urgent_pointer = int(TCP_header[8].hex(), 16)

			print("Source Port : ", TCP_source_port)
			print("Destination Port : ", TCP_destination_port)
			print("Sequence Number : ", TCP_sequence_num)
			print("Acknowledge Number : ", TCP_ack_num)
			print("Data offset : ", TCP_data_offset)
			...
			print("SYN : ", TCP_syn)
			print("FIN : ", TCP_fin)
			print("Window Size : ", TCP_win_size)
			print("TCP checksum : ", TCP_checksum)
			print("Urgent Pointer : ", TCP_urgent_pointer)
		...
		if(Protocol == 17):
			...
			UDP_header = struct.unpack('!2s2s2s2s',packet[0][final_ipv4:final_ipv4+8])
			UDP_source_port = int(UDP_header[0].hex(),16)
			UDP_destination_port = int(UDP_header[1].hex(),16)
			UDP_length = int(UDP_header[2].hex(),16)
			UDP_checksum = int(UDP_header[3].hex(),16)

			print("Source Port : ", UDP_source_port)
			print("Destination Port : ", UDP_destination_port)
